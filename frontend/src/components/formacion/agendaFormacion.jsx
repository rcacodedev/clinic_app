...
const AgendaFormacion = ({ formaciones, fetchFormaciones }) => {
  ...
  const [notificationVisibleEliminar, setNotificationVisibleEliminar] =
    useState(false);
    ...
    const updateDays = generateCalendarDays(month, year).map((dayObj) => {
      ...
      const formacionesDia = Array.isArray(formaciones)
        ? formaciones.filter((f) => {
            ...
            return formacionDays.some(
              (day) => day.toISOString().split("T")[0] === formattedDate
            );
          })
        : [];
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormacionData({ ...formacionData, hora: e.target.value })
                  }
                  className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
      ...
      {confirmDeleteOpen && (
        ...
        >
          <p>
            ¿Estás seguro de que quieres eliminar esta formación? Esta acción no
            se puede deshacer.
          </p>
        </CustomModal>
      )}
